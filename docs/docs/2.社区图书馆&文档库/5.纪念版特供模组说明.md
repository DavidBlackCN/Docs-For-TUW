---
title: 纪念版特供模组说明
createTime: 2026/02/01 20:22:58
permalink: /docs/纪念版特供模组说明/
icon: ic:outline-auto-fix-off
---
::: info 说明
在==纪念版==的更新时，我们新增了一些**特供模组**，同时对一些模组进行了**现代修复**，解决了诸多历史遗留问题，以下为模组的相关说明。
:::

## ==EasyLAN V1.5 Fix==
**By -MF-**  
- 修复了进入EasyLAN设置页面会崩溃的问题
- 修复了游戏内更改EasyLAN设置无法正常保存的问题
- 修复了无法正常输入自定义端口号和自定义玩家人数的问题
- 对EasyLAN进行了局部的现代化重构

<LinkCard icon="simple-icons:modrinth" title="原模组链接" href="https://modrinth.com/mod/easylan" />
<LinkCard icon="ph:link-simple-bold" title="MCMod百科" href="https://www.mcmod.cn/class/11373.html" />

<GitHubCard url=https://github.com/ItIsMF/EasyLAN_Fix />

## ==SoundDisabler V1.0==
**By -MF-**  
- 本模组为纪念版特供，旨在解决标准打包后发生的历史遗留问题
- 通过asm方法阻止原版音乐被播放，避免修改assets本身

<GitHubCard url=https://github.com/ItIsMF/SoundDisabler />

## ==RCsEffect-1.7.10-NEO-Special-Fix==
**By -MF-**   
- 改进了RCE修改药水效果的方法，彻底解决《亡灵战争：弥赛亚》经常出现的 `Ticking Entity` 致命崩溃问题
- 修正了一个平方函数
- 感谢RCE的原作者RC_diamond_GH和ssdsaad
- 注：原始1.7.10版本的RCE已不可考

<GitHubCard url=https://github.com/ItIsMF/RCsEffect_Fix />

## ==RCsEffect-TUW-Special-Fix==
**By DavidBlackCN**

基于 `RCsEffect-1.7.10-NEO-Special-Fix` 的继续修复：

- 修复 BloodPool 处理器将箭、投射物或自定义伤害源直接转换为 EntityLivingBase 时导致的 ClassCastException。
- 修复 XPrepair 为 0 或负数时的除零崩溃，无效数值现在会被安全忽略。
- 为物品使用完成事件增加空值保护，并移除不必要的随机参数解析。
加固来自 NBT 的药水效果数组，安全处理空数组、长度不足、无效药水 ID、持续时间和效果等级。
- 加固外部字符串到数值的转换，避免非法 NBT 或命令参数导致主线程异常。
- 统一击杀事件中的真实攻击者来源，改善近战和远程伤害的兼容性。
- 对 BloodPool 的负数、超界百分比、NaN 和无穷大数值进行防御性处理。
- 将 Player Tick 中的客户端射线检测与攻击逻辑隔离到客户端事件类，降低 Dedicated Server 加载客户端专用类的风险。
- 对 Potion Mixin 的快照迭代器进行最小化稳定性加固，不改写原有药水效果逻辑。

> 修复原则是优先防止崩溃，尽可能保持现有地图的 NBT 格式、玩法和数值行为不变。
<GitHubCard url=https://github.com/DavidBlackCN/RCsEffect-TUW-Special-Fix />

## ==OptiAutoFine==
**By DavidBlackCN**
- 一个 Minecraft 1.7.10 模组，在第一次 Forge 启动时会自动下载 OptiFine，最初设计为TUW团队的地图使用。
- 每条文本和消息都可以通过配置 optiautofine.cfg 进行编辑，可用于语言本地化。
- 没有计划支持更高版本，因为高版本有更好的Optifine替代品。
::: details 示例中文配置
``` cfg
# Configuration file

general {
    # OptiFine jar filename to download. [default: OptiFine_1.7.10_HD_U_E7.jar]
    S:optifineFile=OptiFine_1.7.10_HD_U_E7.jar
}


ui {
    # Continue button text. [default: Continue]
    S:buttonContinue=继续

    # Open mods button text. [default: Open Mods Folder]
    S:buttonOpenMods=打开 Mods 文件夹

    # Quit button text. [default: Quit]
    S:buttonQuit=退出游戏

    # Detail when download is empty. [default: Downloaded file is empty.]
    S:detailDownloadEmpty=下载的文件为空！

    # Detail when download fails. [default: Download failed.]
    S:detailDownloadFailed=§c下载失败！

    # Detail when mcDir is missing. [default: Minecraft directory not available.]
    S:detailMcDirMissing=§cMinecraft 目录不存在！

    # Detail when mods directory cannot be created. [default: Failed to create mods directory.]
    S:detailModsDirFailed=§c创建 Mods 目录失败！

    # Detail when moving the jar fails. [default: Failed to move OptiFine jar into place.]
    S:detailMoveFailed=§c未能将 OptiFine 移动到位！

    # Detail when reason is unknown. [default: Unknown error.]
    S:detailUnknown=§c未知错误！

    # Failure line 1. [default: OptiFine could not be downloaded.]
    S:lineFail1=§c无法下载 OptiFine！

    # Failure line 2. [default: Check the log for details.]
    S:lineFail2=§c请检查游戏日志！

    # Success line 1. [default: OptiFine was downloaded to your mods folder.]
    S:lineSuccess1=§a§lOptiFine 已成功安装！

    # Success line 2. [default: Please restart Minecraft to load it.]
    S:lineSuccess2=§d§l请重启游戏以重新加载 OptiFine！

    # Show detail line. [default: true]
    B:showDetails=true

    # Success detail format, uses %s. [default: File: %s]
    S:successDetailFormat=文件名: %s

    # Title when download fails. [default: OptiFine Download Failed]
    S:titleFail=§6OptiAutoFine 提醒你：

    # Title when download succeeds. [default: OptiFine Installed]
    S:titleSuccess=§6OptiAutoFine 提醒你：
}
```
:::

<GitHubCard url=https://github.com/DavidBlackCN/OptiAutoFine />